@php include_once("php/general-session-variable.php");
	...
	include_once('php/config.php'); @endphp
@php $_SESSION["product-page"] = "true";
	...
	if (isset($_GET["product_id"])) {
		if (mysqli_num_rows($getmaxid_result) == 1) {
			while ($getmaxid_row = mysqli_fetch_assoc($getmaxid_result)) {
				...
				$current_product_id = (int) (clean_input($_GET["product_id"]));
...
@php include_once("php/header.php"); @endphp
...
@php if ($stock_available == "true") { @endphp
...
@php if (isset($_REQUEST['error'])) { @endphp
